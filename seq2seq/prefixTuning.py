...
class PrefixTuning(
        ...
        if not self.optim_prefix:
            ...
            if self.mode_para == 1:
                ...
                self.control_trans = nn.Sequential(
                    ...
                    nn.Linear(
                        self.mid_dim,
                        config.n_layer * 2 * config.n_embd
                    )
                )
                if self.use_infix:
            ...
            # DIFFERENT PARAMETRIZATION:
                ...
                self.control_trans = nn.Sequential(
                    ...
                    nn.Linear(
                        self.mid_dim,
                        self.match_n_layer * 2 * self.n_embd
                    )
                )
                if self.use_infix:
    ...
    def get_prompt_p22(
        ...
        past_key_values = self.control_trans.expand(
            -1,
            bsz,
            -1,
            -1,
    ...
    def lowdata_init_train2(
        ...
        for e in range(
            ...
            our_prompt = self.get_prompt_p5(
                bsz=1
            )
            our_prompt = torch.cat(
                our_prompt,
                dim=0
            )
    ...
    def get_prompt_p2(
        ...
        temp_control = self.dropout(
            temp_control
        )
        past_key_values = temp_control.permute(
            [2, 0, 3, 1, 4]
        ).split(
            2
        )
    ...
    def get_prompt_p3_infix(
        ...
        past_key_values = self.dropout(
            past_key_values
        )
        past_key_values = past_key_values.permute(
            [2, 0, 3, 1, 4]
        ).split(
            2
        )
        ...
        for i in range(
            ...
            full_lst.append(
                torch.cat(
                    [src_past_key_vals[i], past_key_values[i]],
                    dim=3
                )
            )
    ...
    def get_prompt_p5(
        ...
        if self.use_cross_prefix:
            ...
            past_key_values2 = self.dropout(
                past_key_values2
            )
            past_key_values2 = past_key_values2.permute(
                [2, 0, 3, 1, 4]
            ).split(
                2
            )
        ...
        if self.use_encoder_prefix:
            input_tokens_enc = self.input_tokens.unsqueeze(
                0
            ).expand(
                old_bsz,
                -1
            ...
            past_key_values_enc = self.dropout(
                past_key_values_enc
            )
            past_key_values_enc = past_key_values_enc.permute(
                [2, 0, 3, 1, 4]
            ).split(
                2
            )
    ...
    def get_prompt_p1(
        ...
        if control_code is not None:
            ...
            if type(
                ...
                past_key_values = past_key_values.mean(
                    1
                ).unsqueeze(
                    1
                )
                ...
                past_key_values = past_key_values.sum(
                    1
                ).unsqueeze(
                    1
                )
...
class PrefixEmbTuning(
    ...
    def __init__(
        ...
        print(
            'under the PrefixEmbTuning model'
        )
        ...
        if not self.optim_prefix:
            ...
            # DIFFERENT PARAMETRIZATION 2.
                ...
                print(
                    'UNDER PARAMETRIZATION 2'
                )
                tokenizer = GPT2Tokenizer.from_pretrained(
                    'gpt2-medium'
                )
                input_word_lst = [
                    ['name', 'Type', 'price', 'customer rating', 'near', 'area', 'family friendly']
                ]
                input_word_ids = \
                    tokenizer(
                        input_word_lst,
    ...
    def get_prompt_p2(
        ...
        '''
        assert bsz is not None
        temp_control = self.control_trans.unsqueeze(
            0
        ).expand(
            bsz,
            -1,
        ...
        temp_result = gpt2(
            inputs_embeds=temp_control,
            use_cache=True
        )
        past_key_values = temp_result.past_key_values
        return past_key_values

    def get_prompt_p2_infix(
        self,
        src_x,
        control_code=None,
        gpt2=None,
        ...
        temp_control = torch.cat(
            [src_embs, temp_control],
            dim=1
        )
        print(
            temp_control.shape
    ...
    def get_prompt_p5(
        ...
        input_embs = self.control_trans(
            temp_control
        )  # bsz, seqlen, emb_dim
        bsz, seqlen, _ = input_embs.shape
        input_embs = self.dropout(
            input_embs
        )
        temp_result = gpt2(
    ...
    def forward_infix(
        ...
        **kwargs,
        ...
        if self.mode_para == 2:
            ...
            temp_attn = torch.ones(
                bsz,
                self.preseqlen
            ).bool()
            attention_mask = torch.cat(
                [src_attn, temp_attn, tgt_attn],
                dim=1
            )
