...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='data2text E2E training args.'
    )
    parser.add_argument(
        '--mode',
        type=str,
    ...
    if args.dir_name is None:
        Model_FILE = args.mode + args.tuning_mode + '_' + args.optim_prefix[:1] + '_' + str(
            args.preseqlen
        ) + \
    ...
    if args.prefix_mode == 'embedding':
        app += ' --parametrize_emb {} '.format(
            args.parametrize_emb
        )

    if args.tuning_mode == 'adaptertune':
        app += ' --adapter_design {} '.format(
            args.adapter_design
        )
    ...
    if args.do_train == 'yes':
        ...
        if args.tuning_mode == 'finetune':
            ...
            print(
                'loading from the finetune model {}'.format(
                    args.finetune_model_path
                )
            )
            Model_FILE = args.finetune_model_path + '_decode_eval' + '_{}'.format(
                args.length_pen
            )
            print(
                'writing the decoded results to {}'.format(
                    Model_FILE
                )
            )
            COMMANDLINE = 'python finetune.py ' \
            ...
            print(
                'loading from the prefix model {}'.format(
                    args.prefix_model_path
                )
            )
            print(
                'loading from the main model {}'.format(
                    OLD_MODEL
                )
            )
            Model_FILE = args.prefix_model_path + '_decode_eval' + '_{}'.format(
                args.length_pen
            )
            print(
                'writing the decoded results to {}'.format(
