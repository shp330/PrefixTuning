...
arg_to_scheduler_choices = sorted(
    arg_to_scheduler.keys()
)
arg_to_scheduler_metavar = "{" + ", ".join(
    arg_to_scheduler_choices
) + "}"


class OurModelCheckPoint(
        ...
        **config_kwargs
        ...
        if seq2seq_model is None:
            self.seq2seq_model = BartForConditionalGeneration.from_pretrained(
                ...
                from_tf=bool(
                    ".ckpt" in self.hparams.model_name_or_path
                ),
                config=self.config,
                cache_dir=cache_dir,
            )
        ...
        if self.hparams.prefixModel_name_or_path is not None:
            print(
                'loading from {}'.format(
                    hparams.prefixModel_name_or_path
                )
            )
        ...
        return os.path.join(
            ...
            "cached_{}_{}_{}".format(
                ...
                list(
                    filter(
                        None,
                        self.hparams.model_name_or_path.split(
                            "/"
                        )
                    )
                ).pop(),
...
class LoggingCallback(
    ...
    def on_batch_end(
        ...
        lrs = {f"lr_group_{i}": lr for i, lr in enumerate(
            lr_scheduler.get_lr()
        )}
...
def generic_train(
    ...
    **extra_train_kwargs
    ...
    print(
        'the max number of epochs is {}'.format(
            args.max_epochs
        )
    )
    print(
