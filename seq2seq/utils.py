...
def label_smoothed_nll_loss(
    ...
    nll_loss = -lprobs.gather(
        dim=-1,
        index=target
    )
    smooth_loss = -lprobs.sum(
        dim=-1,
        keepdim=True
    )
    if ignore_index is not None:
        ...
        smooth_loss = smooth_loss.squeeze(
            -1
        )

    nll_loss = nll_loss.sum()  # mean()? Scared to break other math.
    smooth_loss = smooth_loss.sum()
    eps_i = epsilon / lprobs.size(
        -1
    )
...
class AbstractSeq2SeqDataset(Dataset):
    def __init__(
        ...
        if os.path.exists(
            ...
            self.src_lens = pickle_load(
                self.len_file
            )
            self.used_char_len = False
        else:
            self.src_lens = self.get_char_lens(
                self.src_file
            )
        ...
        assert min(
            self.src_lens
        ) > 0, f"found empty line in {self.src_file}"
        ...
        self.add_prefix_space = isinstance(
            self.tokenizer,
            BartTokenizer
        )
    ...
    def make_dynamic_sampler(
        ...
        **kwargs
        ...
        sorted_indices = list(
            self.make_sortish_sampler(
                1024,
                shuffle=False
            )
        )

        def num_tokens_in_example(
            i
        ):
            return min(
                self.src_lens[i],
        ...
        approximate_toks_per_batch = [max(
            self.src_lens[i] for i in batch
        ) * len(
            batch
        ) for batch in shuffled_batches]
        largest_batch_idx = np.argmax(
...
def sortish_sampler_indices(
    ...
    sz = bs * 50
    ck_idx = [idxs[i: i + sz] for i in range(
        0,
        len(
            idxs
        ),
    ...
    sz = bs
    ck_idx = [sort_idx[i: i + sz] for i in range(
        0,
        len(
            sort_idx
        ),
...
class DistributedSortishSampler(
    ...
    def available_indices(
        ...
        indices += indices[: (self.total_size - len(
            indices
        ))]
        assert len(
            indices
...
def use_task_specific_params(
    ...
    if task_specific_params is not None:
        pars = task_specific_params.get(
            task,
            {}
        )
        logger.info(
            f"using task specific params for {task}: {pars}"
        )
        model.config.update(
            pars
        )
...
def flatten_list(
    ...
    return [x for x in itertools.chain.from_iterable(
        summary_ids
    )]
...
def calculate_rouge(
    ...
    for reference_ln, output_ln in zip(
        reference_lns,
        output_lns
    ):
        scores = scorer.score(
...
def assert_all_frozen(
    model
):
    model_grads: List[bool] = list(
        grad_status(
            model
        )
    )
