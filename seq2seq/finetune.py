...
class PrefixSummarizationModule(
    PrefixTransformer
):
    mode = "summarization"
    loss_names = ["loss"]
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.metrics_save_path = Path(
            self.output_dir
        ) / "metrics.json"
        self.hparams_save_path = Path(
            self.output_dir
        ) / "hparams.pkl"
        pickle_save(
            self.hparams,
    ...
    def _step(
        ...
        if isinstance(
            ...
            decoder_input_ids = self.model._shift_right(
                tgt_ids
            )
        else:
    ...
    def training_step(
        ...
        logs["src_pad_tok"] = batch["input_ids"].eq(
            self.pad
        ).sum()
        logs["src_pad_frac"] = batch["input_ids"].eq(
            self.pad
        ).float().mean()

        # print('hi', loss_tensors[0].item())
        self.training_acc_across_batches_at_curr_epoch.append(
            loss_tensors[0].item()
        )
    ...
    def validation_epoch_end(
        ...
        losses = {k: torch.stack(
            [x[k] for x in outputs]
        ).mean() for k in self.loss_names}
        loss = losses["loss"]
        print(
            loss
        )
        generative_metrics = {
            k: np.array(
                [x[k] for x in outputs]
            ).mean() for k in self.metric_names + ["gen_time", "gen_len"]
        }
        metric_val = (
            generative_metrics[self.val_metric] if self.val_metric in generative_metrics else
            losses[self.val_metric]
        )
    ...
    def _generative_step(
        ...
        preds: List[str] = self.ids_to_clean_text(
            generated_ids
        )
        target: List[str] = self.ids_to_clean_text(
            batch["labels"]
        )
        loss_tensors = self._step(
            batch
        )
        base_metrics = {name: loss for name, loss in zip(
            self.loss_names,
            loss_tensors
        )}
        # print('INPUT:', self.ids_to_clean_text(batch["input_ids"]))
        # print(preds, target)
        rouge: Dict = self.calc_generative_metrics(
            preds,
            target
        )
        summ_len = np.mean(
    ...
    def add_model_specific_args(
        ...
        PrefixTransformer.add_model_specific_args(
            parser,
            root_dir
        )
        add_generic_args(
    ...
    if model is None:
        if "summarization" in args.task_mode:
            if args.tuning_mode == 'prefixtune':
                model = PrefixSummarizationModule(
                    args
                )
            ...
            model: SummarizationModule = TranslationModule(
                args
            )
...
if __name__ == "__main__":
    ...
    parser = PrefixSummarizationModule.add_model_specific_args(
        parser,
        os.getcwd()
    )
